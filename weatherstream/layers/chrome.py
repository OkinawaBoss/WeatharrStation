...
class ChromeLayer(Layer):
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(0, 0, width, height, min_interval=min_interval)
        self.location = location_name
        self.title = title
        self.font_large = _font(68)
        self.font_small = _font(42)
    ...
    def tick(self, now: float):
        ...
        # Title + location
        draw.text((64, 52), self.title, font=self.font_large, fill=(235, 242, 255, 255))
        draw.text((64, 120), self.location, font=self.font_small, fill=(210, 220, 230, 255))

        # NOAA logo centered
        self._load_logo()
        if self._logo:
            try:
                logo = self._logo.resize((108, 108), Image.LANCZOS)
            except Exception:
                logo = self._logo
        ...
        # Ticker tray accent
        tray_h = 64
        tray_rect = (48, self.surface.height - tray_h - 20, self.surface.width - 48, self.surface.height - 20)
        draw.rounded_rectangle(tray_rect, radius=24, fill=(16, 24, 40, 235))
